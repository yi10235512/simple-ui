...
export const headerPlugin = (md: MarkdownIt) => {
  ...
  md.render = (content, env) => {
    ...
    console.log('env.headers', env.headers)
    //  if (!env.headers) {
    //   env.headers = extractHeaders(content) // 手动解析 headers
    // }
...
function processHeaders(headers: AugmentedHeader[]) {
  for (const h of headers) {
    console.log('h', h.title)
    if (h.title.endsWith('*')) {
      if (h.title.endsWith('**')) {
        console.log('compostiotion')
