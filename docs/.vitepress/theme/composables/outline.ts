import { getScrollOffset, Header, useData } from 'vitepress'
import type { DefaultTheme } from 'vitepress/theme'
import { inject, onMounted, onUnmounted, onUpdated, computed, ref, type Ref } from 'vue'
...
export function useOutlineHeaders() {
  ...
  return computed(() => {
    return resolveHeadersByFilter(page.value.headers, filterHeaders)
  })
}
function resolveHeadersByFilter(
  headers: MyHeader[],
  filter?: (h: MyHeader) => boolean
): MyHeader[] {
...
export function resolveHeaders(
  ...
  console.log('resolveHeaders func', headers, range)
  if (range === false) {
    return []
  }
